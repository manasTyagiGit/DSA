...
{
    if ((head == NULL) || (head -> next == head))           return NULL;            // 0 or 1 nodes exist, so no prevNode
    
    if ((head -> next) -> value == key)                     return head;            // head is the prevNode
...
void push (linkedList** head, int key)
{
    ...
    if (iter != NULL)                 // linked list is not empty
    {
...
void deleteNode (linkedList** head, int key)
{
    ...
    if (node != NULL)
    {
        ...
        if ((predNode == NULL))                       // only one element, i.e., head exists
        {
            (*head) = NULL;                        
        }

        else if (node == (*head))                     // want to remove head
        {
            (*head) = (*head) -> next; 
            (predNode -> next) = (*head);             // we have to add this all the time as references need to be reset                     
        }
        
        else                                         // any node to remove in middle or end
        {
...
int main (void)
{
    ...
    printf ("\nWelcome to the simulator for a Circular Linked List\n");
    printf ("------------------------------------------\n");
